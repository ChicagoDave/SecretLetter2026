...
namespace Textfyre.VM
{
    ...
    internal delegate Task OpcodeHandler(uint[] operands);
    ...
    internal class Opcode
    {
        ...
        private readonly Func<uint[], Task> handler;

        public Opcode(OpcodeAttribute attr, Func<uint[], Task> handler)
        {
            this.attr = attr;
    ...
    public partial class Engine
    {
        ...
        private async Task op_save(uint[] args)
        {
            if (nestingLevel == 0 && SaveRequestedAsync != null)
            {
                SaveRestoreEventArgs e = new SaveRestoreEventArgs();
                await SaveRequestedAsync(this, e);
                if (e.Stream != null)
                {
                    try
                    {
        ...
        private async Task op_restore(uint[] args)
        {
            if (LoadRequestedAsync != null)
            {
                SaveRestoreEventArgs e = new SaveRestoreEventArgs();
                await LoadRequestedAsync(this, e);
                if (e.Stream != null)
                {
                    try
                    {
        ...
        private async Task op_fyrecall(uint[] args)
        {
            args[3] = 0;
            ...
            switch (call)
            {
                case FyreCall.ReadLine:
                    DeliverOutput();
                    await InputLineAsync(args[1], args[2]);
                    break;

                case FyreCall.ReadKey:
                    DeliverOutput();
                    args[3] = (uint)(await InputCharAsync());
                    break;
