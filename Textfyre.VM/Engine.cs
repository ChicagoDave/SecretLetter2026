...
namespace Textfyre.VM
{
    ...
    public partial class Engine
    {
        ...
        private void InitOpcodeDict()
        {
            ...
            foreach (MethodInfo mi in methods)
            {
                ...
                if (attrs.Length > 0)
                {
                    ...
                    Func<uint[], Task> handler;
                    if (mi.ReturnType == typeof(Task))
                    {
                        handler = (Func<uint[], Task>)Delegate.CreateDelegate(typeof(Func<uint[], Task>), this, mi);
                    }
                    else
                    {
                        var syncHandler = (Action<uint[]>)Delegate.CreateDelegate(typeof(Action<uint[]>), this, mi);
                        handler = (args) => { syncHandler(args); return Task.CompletedTask; };
                    }
                    opcodeDict.Add(attr.Number, new Opcode(attr, handler));
                }
            }
        }
        ...
        // Async event delegates for WASM (single-threaded) compatibility
        public Func<object, LineWantedEventArgs, Task> LineWantedAsync;
        public Func<object, KeyWantedEventArgs, Task> KeyWantedAsync;
        public Func<object, SaveRestoreEventArgs, Task> SaveRequestedAsync;
        public Func<object, SaveRestoreEventArgs, Task> LoadRequestedAsync;
        ...
        /// <summary>
        /// Starts the interpreter asynchronously for WASM single-threaded compatibility.
        /// </summary>
            ...
            while (running)
            {
                switch (execMode)
                {
                    case ExecutionMode.Code:
                        ...
                        // call opcode implementation (sync path: NestedCall only uses sync opcodes)
                        pc = operandPos; // after the last operand
                        var _ = opcode.Handler(operands);
        ...
        /// <summary>
        /// Runs the main interpreter loop asynchronously for WASM single-threaded compatibility.
        /// The only difference from InterpreterLoop is that opcode handlers are awaited.
        /// </summary>
        private async Task InterpreterLoopAsync()
        {
            const int MAX_OPERANDS = 8;
            uint[] operands = new uint[MAX_OPERANDS];
            uint[] resultAddrs = new uint[MAX_OPERANDS];
        ...
        private async Task InputLineAsync(uint address, uint bufSize)
        {
            string input = null;
            ...
            // ask the application to read a line
            await LineWantedAsync(this, lineArgs);
            input = lineArgs.Line;
        ...
        private async Task<char> InputCharAsync()
        {
            // can't do anything without this event handler
            if (KeyWantedAsync == null)
                return '\0';

            KeyWantedEventArgs keyArgs = new KeyWantedEventArgs();

            // ask the application to read a character
            await KeyWantedAsync(this, keyArgs);
            return keyArgs.Char;
        }
